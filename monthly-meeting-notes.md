...
* Shreyas Khandekar (today's presenter)
* Michelle Strout
* Paul Sathre
* Engin Kayraklioglu
* Abhishek Girish
...
Shreyas Khandekar presented on [fastotf2](https://github.com/hpc-ai-adv-dev/fastotf2), a Chapel implementation
of a subset of the Open Trace Format 2 (OTF2) library which is a standardized trace format used by tools such
as [Score-P](https://www.vi-hps.org/projects/score-p) for profiling and event tracing of HPC applications. The
focus is on a tool for converting OTF2 traces to CSV for telemetry analysis. By leveraging Chapel's
parallelism and C interoperability, it replaces the Python workflows to speed up trace processing.

- this is a collaboration with Oak Ridge to process tracing files generated by Score-P
  - ultimately, they are interested in monitoring power ussage on Frontier cluster while running HPL
    benchmarks
- fastotf2 library converts these traces to CSV
- each thread gets its own event file (these are the main core of the OTF2 trace) times the number of locales
  / MPI tasks
- Chapel API is a very thin wrapper over the C API
  - slightly more usable than C: better data structures + distribution across nodes
- for more details, check the `*.md` files at https://github.com/hpc-ai-adv-dev/fastotf2
...
Engin: Chapel recently became part of the High Performance Software Foundation https://hpsf.io which has a
training discussion group (do they have similar training meetups?). It would be great to maybe join our forces
or at least to pick up their brains on how we could teach / popularize Chapel better within their
ecosystem. The person in charge of training on their end might have been Fernanda Foertter (ORNL). Engin will
post in their Slack channel and communicate back.

Brad, Michelle, Engin, Alex will discuss this offline before a wider call for a new chair.
